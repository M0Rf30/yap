...
<!-- Updated: 2026-05-08T23:33:31.768Z -->
...
## todos
```
pkg/builders/rpm/rpm.go:294  # TODO: Implement changelog support when rpmpack adds the necessary API.
...
```
pkg/archive/tar_test.go                       +TestCreateTarCompressedWithGzip  +TestCreateTarCompressedWithXz  +TestCreateTarCompressedWithZstd  +TestCreateTarCompressedInvalidCompression
pkg/builders/apk/apk_test.go                  ~TestBuildPackage
pkg/builders/common/extract_test.go           +buildTarGz  +writeARMember
pkg/builders/pacman/makepkg_test.go           +TestPrepareFakeroot_WithScriptlets  +TestPrepareFakeroot_WithoutScriptlets  +TestPrepareFakeroot_WithChangelog  ~TestBuildPackage
pkg/builders/deb/dpkg_test.go                 +TestCreateChangelogFile  +TestCreateChangelogFile_NoChangelog  ~createTestPKGBUILD  ~TestNewBuilder
pkg/builders/rpm/rpm_test.go                  ~TestBuildPackage  ~TestBuildPackageWithoutEpoch  ~TestProcessDepends  ~TestExtractFileModTimeUint32
pkg/parser/additional_parser_test.go          +TestParseFileWithUpgradeScriptlets  ~TestParseFileErrors
...
pkg/pkgbuild/pkgbuild_test.go                 +TestSetupSysrootEnvironment_DiscoverCustomPrefix  ~TestPKGBUILD_AddItem  ~TestSetupSysrootEnvironment_PrependsToExisting
pkg/pkgbuild/changelog_test.go                +TestReadChangelog_Empty  +TestReadChangelog_FileNotFound  +TestReadChangelog_Success  +TestReadChangelog_AbsolutePath
pkg/sbom/integration_test.go                  +TestSBOMIntegration  +TestSBOMWithMinimalPackage  +TestSBOMFormatSelection
pkg/project/utils_test.go                     +TestFindGitRoot
pkg/sbom/sbom_test.go                         +TestGenerateToFiles  +TestGenerateEmptyFormats  +TestValidateOptions
pkg/sbom/sbom_internal_test.go                +TestExtractDepName  +TestGeneratePurl  +TestGenerateDocumentNamespace  +TestGenerateCycloneDX
pkg/signing/gpg_test.go                       +generateTestGPGKey  +TestNewGPGSignerInvalidKey  +TestNewGPGSignerMissingFile  +TestGPGSignerSignDEB
pkg/signing/gpg_internal_test.go              +generateTestGPGKey  +TestNewGPGSignerValidKey
...
pkg/signing/rsa_internal_test.go              +generateTestRSAKey  +createFakeAPK  +extractSignatureFromAPK  +TestNewRSASignerValidKey
pkg/signing/rsa_test.go                       +createFakeAPK  +generateTestRSAKey  +TestNewRSASignerInvalidPEM  +TestNewRSASignerMissingFile
cmd/yap/command/build_test.go                 +TestValidateCompression  ~TestBuildCommandDefinition
pkg/signing/signing_internal_test.go          +TestResolveKeyPathPriority  +TestResolvePassphrasePriority  +TestAlgorithmForFormat
pkg/signing/signing_test.go                   +TestNewSignerDisabled  +TestNewSignerEnabled  +TestNewSignerEnabledNoKey
pkg/archive/tar.go                            +CreateTarCompressed  +CreateTarZst  +ExtractFiltered  ~CreateTarZst
pkg/builders/apk/apk.go                       ~NewBuilder
pkg/builders/common/interface.go              +that  +so  ~that  ~so
pkg/builders/deb/dpkg.go                      +NewBuilder  ~NewBuilder
pkg/builders/rpm/rpm.go                       +NewBuilder  +extractFileModTimeUint32  ~NewBuilder  ~extractFileModTimeUint32
pkg/builders/pacman/makepkg.go                ~NewBuilder
pkg/packer/packer.go                          +GetPackageManager  ~GetPackageManager
pkg/pkgbuild/pkgbuild.go                      +isIncludeDir  +isLibDir  +isPkgConfigDir  +appendDiscoveredDirs
pkg/project/project.go                        +signingFormatForArtifact  +findGitRoot
pkg/sbom/cyclonedx.go                         +generateCycloneDX  +generatePurl  +extractDepName
pkg/sbom/sbom.go                              +Generate  +ValidateOptions
pkg/sbom/spdx.go                              +generateSPDX  +generateDocumentNamespace  +getDownloadLocation
pkg/signing/rsa.go                            +NewRSASigner  +parseRSAPrivateKey  +extractFirstGzipStream
pkg/shell/archive_handler.go                  +archiveExecHandler  +handleUnzip  +parseGunzipArgs  +handleGunzip
pkg/signing/factory.go                        +NewSigner
pkg/signing/resolve.go                        +resolveAndValidateKeyPath  +Resolve  +resolveKeyPath  +resolvePassphrase
pkg/signing/gpg.go                            +for  +NewGPGSigner  +suitable  +takes
pkg/shell/exec.go                             ~RunScriptWithPackage
cmd/yap/command/build.go                      +validateCompression  +resolveSigning  ~InitializeBuildDescriptions  ~init
.github/copilot-instructions.md               +TestLogStructuredError  +TestBuildCommandDefinition  +TestValidateCompression  +NewRootCmd
```
...
func (s) Sign(ctx context.Context, artifactPath string) → error
func (s) SignRPM(data []byte) → ([]byte, error)
...
func (s) GetSigningFunction() → func([]byte) ([]byte, error)
```
