...
<!-- Updated: 2026-05-17T13:27:00.410Z -->
...
```
pkg/aptcache/aptcache_test.go                 +newCacheFromStrings  +TestLookup_AptIndex  +TestLookup_DpkgStatus  +TestMerge_AptThenDpkg
pkg/aptcache/fuzz_test.go                     +FuzzParseDeb822  +FuzzParseDeb822DpkgStatus  +FuzzLookupAfterParse  +FuzzResolveVirtual
pkg/aptcache/export_test.go                   +NewCacheForTesting
pkg/binary/strip_test.go                      +writeMinimalELF  +TestStripSkipsForeignArchELFOnFallback  ~contains
pkg/builders/apk/apk_test.go                  ~TestCreateAPKPackage
...
pkg/errors/fuzz_test.go                       +FuzzErrorNew  +FuzzErrorWrap  +FuzzWithContext  +FuzzErrorChaining
pkg/download/progress_test.go                 +TestHumanBytes  +TestFmtDuration  ~TestProgressBarRender  ~TestProgressBarPercentageCalculation
pkg/gensum/fuzz_test.go                       +countHashSlots  +isHashOrSkip  +FuzzReplaceChecksumValues  +FuzzReplaceHashesInBlock
pkg/gensum/export_test.go                     +ReplaceChecksumValuesExported  +ExtractSourceBlocksExported  +ParseArrayValuesExported  +ReplaceHashesInBlockExported
pkg/gensum/gensum_test.go                     +writePKGBUILD  +readPKGBUILD  +TestReplaceHashesInBlock_SingleLine  +TestReplaceHashesInBlock_MultiLine
pkg/graph/loader/fuzz_test.go                 +FuzzParseDependencyLine  +FuzzCleanDependencyName  +FuzzParseDependencyArray  +FuzzLoadProjectForGraph
pkg/i18n/fuzz_test.go                         +FuzzTranslate  +FuzzTranslateWithData  +FuzzDetectLanguage  +FuzzInitWithLanguage
pkg/logger/logger_test.go                     +TestMultiPrinterStart  +TestSetWriter  ~TestArgsToLoggerArgs  ~TestYapLoggerArgs
pkg/parser/fuzz_test.go                       +FuzzParsePKGBUILD  +FuzzParsePKGBUILDVariables
pkg/pkgbuild/fuzz_test.go                     +FuzzParseDirective  +FuzzAddItemString  +FuzzAddItemArray  +FuzzCheckLicense
...
pkg/project/utils_test.go                     ~TestGetMakeDepsDeduplication  ~TestGetRuntimeDepsDeduplication
pkg/project/project_test.go                   +TestBuildOptions  +TestBuildOptionsModification  ~TestGlobalVariables  ~TestGlobalVariableModification
pkg/source/fuzz_test.go                       +FuzzParseURI  +FuzzGetProtocol  +FuzzSourceItemPathValidation  +FuzzRefKeyAndValue
...
cmd/yap/command/status_test.go                ~TestContains  ~TestStatusCommandDefinition  ~TestGetDistroFamily
cmd/yap/command/version_test.go               ~TestGetCurrentTime
pkg/aptcache/aptcache.go                      +Load  +loadFromDisk  +applyField
pkg/binary/strip.go                           +hostElfMachine  +isForeignArchELF  ~StripLTO
pkg/builders/common/cross.go                  ~isHostOnlyPackage  ~partitionArchAllDeps  ~partitionArchAllDepsForExtract
pkg/builders/common/interface.go              ~getUpdateCommand
pkg/builders/rpm/rpm.go                       ~NewBuilder
pkg/builders/pacman/makepkg.go                ~NewBuilder
pkg/color/color.go                            +Disable  +Enable  +IsDisabled  +wrap
pkg/download/download.go                      ~Download  ~logDownloadStart
pkg/download/progress.go                      +NewProgressBar  +logPrefix  +fmtDuration  +humanBytes
pkg/gensum/gensum.go                          +UpdateChecksums  +extractArrayBlocks  +parseArrayValues  +computeHashes
pkg/options/apply.go                          +in  ~Apply
pkg/logger/logger.go                          +visibleLen  +termWidth  +avoids  +getHandler
pkg/options/libtool.go                        +removeByExtension  +RemoveLibtool  ~RemoveLibtool
pkg/options/static.go                         +RemoveStatic
pkg/packer/packer.go                          ~GetPackageManager
pkg/pkgbuild/pkgbuild.go                      ~resolveVirtualPackages
pkg/shell/exec.go                             ~formatDecoratedLine  ~logScriptContent
pkg/signing/resolve.go                        +ResolveGeneric  +resolveGenericKeyPath  +resolveGenericPassphrase  +findGenericDefaultKey
cmd/yap/command/build.go                      ~resolveSigning  ~InitializeBuildDescriptions  ~init
cmd/yap/command/help_formatting.go            ~SetupEnhancedHelp
cmd/yap/command/gensum.go                     +InitializeGensumDescriptions  +init
cmd/yap/command/prepare.go                    ~init
cmd/yap/command/usage.go                      ~CustomUsageFunc  ~printOrganizedFlags  ~printFlag
cmd/yap/command/status.go                     ~contains  ~showSystemStatus
cmd/yap/command/smart_help.go                 ~SmartErrorHandler  ~provideSimilarCommands  ~provideSimilarDistributions  ~showCommandGroups
cmd/yap/command/root.go                       +ParseLanguageFlag  ~Execute  ~InitializeLocalizedDescriptions  ~updateSubCommandDescriptions
cmd/yap/command/zap.go                        ~init
cmd/yap/command/version.go                    +printVersion  ~getCurrentTime
.github/copilot-instructions.md               +TestStatusCommand  +TestStatusCommandDefinition  +TestGetDistroFamily  +TestCheckContainerRuntime
```
...
h2 cmd
h3 cmd/yap/command/status_test.go
h3 cmd/yap/command/version_test.go
...
### cmd/yap/command/status_test.go
```
...
### cmd/yap/command/version_test.go
```
...
func (c) loadAptLists(dir string) → error
func (c) loadDpkgStatus(path string) → error
func (c) parseFile(path string, dpkgStatus bool) → error
func (c) parseDeb822(r io.Reader, dpkgStatus bool) → error
func applyField(s *stanza, field, value string, dpkgStatus bool)
func (c) flushStanza(s stanza, dpkgStatus bool)
func (c) flushProvides(pkgName, provides string)
```
...
func computeHashes(uris []string, tmpDir string) → ([]string, error)
func downloadAndHash(rawURI, uri, tmpDir string) → (string, error)
