...
func (src *Source) Get() error {
	...
	case "http", "https", "ftp", constants.Git:
		if !utils.Exists(sourceFilePath) {
			src.getURL(sourceType, sourceFilePath, SSHPassword)
		}
	case "file":
	default:
...
func (src *Source) getURL(protocol, dloadFilePath, sshPassword string) {
	if protocol != constants.Git {
		utils.Download(dloadFilePath, src.SourceItemURI)
	} else {
		referenceName := src.getReferenceType()
		utils.GitClone(src.SourceItemURI, dloadFilePath, sshPassword, referenceName)
	}
}
