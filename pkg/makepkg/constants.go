...
{{- range . }}
{{- if eq .Type "dir" }}
.{{ .Destination }} time={{ .Time }}.0 type=dir
{{- else if eq .Type "symlink" }}
.{{ .Destination }} time={{ .Time }}.0 mode={{ printf "%o" .Mode }} type=link link={{ .LinkSource }}
{{- else }}
{{- if eq .Destination "/.BUILDINFO" }}
.{{ .Destination }} time={{ .Time }}.0 size={{ .Size }} sha256digest={{ printf "%x" .SHA256 }}
{{- else if eq .Destination "/.PKGINFO" }}
.{{ .Destination }} time={{ .Time }}.0 size={{ .Size }} sha256digest={{ printf "%x" .SHA256 }}
/set mode=755
{{ else }}
.{{ .Destination }} time={{ .Time }}.0 mode={{ printf "%o" .Mode }} size={{ .Size }} sha256digest={{ printf "%x" .SHA256 }}
{{- end }}
{{- end }}
{{- end }}
`
